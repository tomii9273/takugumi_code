...
def make_takugumi(n_player: int) -> bool:
    """
    n_player 人の抜け番あり 8 + 1 戦の卓組の作成を試みる。作成できたら True を返す。
    参考: https://tomii6614.web.fc2.com/nukeari_8and1_sen_method.html
    """
    assert n_player >= 39
    ...
    if k in (1, 2):
        ...
        # 抜け番経験者に最終戦の卓番号を割り当てる
        nukeban_cnt = 0
        for p in range(n_player):
            if -1 in new_takugumi[:-1, p]:
                new_takugumi[-1, p] = nukeban_cnt // GAME_N + 1
                nukeban_cnt += 1
    ...
    array_to_txt(new_takugumi, f"{n_taku}taku_{n_player}nin_8and1sen.txt")
    return True
