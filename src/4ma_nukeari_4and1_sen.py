...
def check_loaded_takugumi_teamsen(takugumi: np.ndarray, taku_check: bool = False) -> None:
    """
...
def array_to_txt(takugumi: np.ndarray) -> None:
    ...
    n_player = takugumi.shape[1]
    n_taku = n_player // GAME_N
    with open(f"{n_taku}taku_{n_player}nin_4and1sen.txt", "w") as f:
...
def make_takugumi(n_player: int) -> bool:
    ...
    if k in (1, 2):
        ...
        takugumi = takugumi_to_array(
            TAKUGUMI_DIR + "/" + select_file(f"{n_taku}taku_{GAME_N*n_taku}nin", "4team.txt")
        )[:4, :]
        ...
        takugumi = takugumi_to_array(
            TAKUGUMI_DIR + "/" + select_file(f"{n_taku+1}taku_{GAME_N*(n_taku+1)}nin", "4team.txt")
        )[:4, :]
        check_loaded_takugumi_teamsen(takugumi, taku_check=False)
