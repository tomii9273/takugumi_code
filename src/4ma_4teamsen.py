...
def chofuku_check_c(n: int) -> bool:
    """チーム C (3 チーム目) の n 戦目について、過去 n-1 戦と重複対戦がなければ True を返す。"""
    for i in range(n):
...
def chofuku_check_d(n: int) -> bool:
    """チーム D (4 チーム目) の n 戦目について、過去 n-1 戦と重複対戦がなければ True を返す。"""
    for i in range(n):
        if ((top_C[i] - top_D[i]) - (top_C[n] - top_D[n])) % n_taku == 0:
            return False
        if ((top_D[i] - top_B[i]) - (top_D[n] - top_B[n])) % n_taku == 0:
            return False
    return True


def dfs_top_D(now_taisen: int, taku_no_list: list[int]) -> bool:
    """
    チーム D (4 チーム目) の先頭の選手の卓番号を深さ優先探索で探索する。
    ...
    if now_taisen == n_taisen:
        ...
        for taku_no in taku_no_list:
            top_D[now_taisen] = taku_no
            if chofuku_check_d(now_taisen):
                taku_no_list_new = taku_no_list[:]
                taku_no_list_new.pop(taku_no_list.index(taku_no))
                if dfs_top_D(now_taisen + 1, taku_no_list_new):
                    return True
        top_D[now_taisen] = -1
    return False
...
def dfs_top_C(now_taisen: int, taku_no_list: list[int]) -> bool:
    ...
    最終戦まで求められたら チーム D (4 チーム目) の探索を開始する。
    now_taisen: 現在の深さ (now_taisen 戦目 (0-indexed) を探索する)
    taku_no_list: now_taisen 戦目以降で使うことのできる卓番号のリスト
    """
    if now_taisen == n_taisen:
        return dfs_top_D(1, list(range(1, n_taku)))
    else:
        for taku_no in taku_no_list:
            top_C[now_taisen] = taku_no
            if chofuku_check_c(now_taisen):
                taku_no_list_new = taku_no_list[:]
                taku_no_list_new.pop(taku_no_list.index(taku_no))
...
def maketxt() -> None:
    """2, 3, 4 チーム目の先頭の選手の卓番号の配列 top_B, top_C をもとに、卓組 txt ファイルを作成"""
    takugumi = [[0] * (4 * n_taku) for _ in range(n_taisen)]
    for s in range(n_taisen):
        for p in range(n_taku):
            ...
            takugumi[s][p + 3 * n_taku] = (p + top_D[s]) % n_taku

    # 卓番号を 1-indexed に変更
    for s in range(n_taisen):
        for p in range(4 * n_taku):
            takugumi[s][p] = takugumi[s][p] + 1

    # txt として保存
    with open(f"{n_taku}taku_{4*n_taku}nin_{n_taisen}sen_4team.txt", "w") as f:
        for p in range(4 * n_taku):
            for s in range(n_taisen):
...
def make_takugumi() -> bool:
    """
    4 人麻雀 4 チーム戦を、n_taku 卓で n_taisen 戦分作成する。
    見つかったら True を返す。
    参考: https://tomii6614.web.fc2.com/4team_sen_method.html の ②
    """
    assert 4 <= n_taku  # 3 卓以下では 1 戦分しか作成できない
    assert 1 <= n_taisen <= n_taku

    result = False
    for comb_from_1 in list(itertools.combinations(range(1, n_taku), n_taisen - 1)):
        global top_B, top_C, top_D
        # チーム B (2 チーム目) の先頭の選手の卓番号は「0, 1, 2, ..., n_taku-1 から必ず 0 を含めて n_taisen 個選んだもの」を全て調べる
        top_B = [0] + list(comb_from_1)
        # チーム C, D (3, 4 チーム目) の先頭の選手の卓番号は関数 dfs_top_C で探索する
        top_C = [-1] * n_taisen
        top_C[0] = 0
        top_D = [-1] * n_taisen
        top_D[0] = 0
