...
卓組を作成できた場合、同じディレクトリに卓組ファイル ((縦軸がプレイヤー・横軸が対戦の) タブ区切りテキストファイル、または html ファイル) を作成し、標準出力に `found` と実行時間を出力し、終了します。  
卓組を作成できなかった場合、標準出力に `notfound` と実行時間を出力し、終了します。  
実行時間は以下の環境で実測 (1 回測定) したもの、または他の環境での実測後に以下の環境での時間を推定したものです。あくまで目安です。
...
`python guest.py [ゲスト数g] [卓数T] [対戦数S]`

$4$ 人麻雀で、$g$ 人のゲスト選手を招いた $T$ 卓での対局について $S$ 戦分の作成を試みます。  
$3gS≧4T-g$ である必要があります。

### 実行結果

| ゲスト数 $g$ | 卓数 $T$ | 対戦数 $S$ | 作成可否 | 実行時間 [min] |
| ------------ | -------- | ---------- | -------- | -------------: |
| 1            | 2        | 3 ～ 8     | 可       |              1 |
| 1            | 3        | 4 ～ 8     | 可       |              2 |
| 1            | 4        | 6 ～ 8     | 可       |         1 ～ 3 |
...
[4 人麻雀 ゲストを呼ぶ大会 卓組作成方法](https://tomii6614.web.fc2.com/guest_method.html)
